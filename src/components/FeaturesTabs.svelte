<script lang="ts">
  ...
  <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
    {#each features as feature}
        {#snippet icon()}
            <Fa icon={feature.icon} color="primary" class="inline-block"/>
        {/snippet}
        <FeatureCard {icon} title={feature.title} class={feature.id === $currentTab ? 'active' : ''} onClick={() => $currentTab = feature.id}/>
    {/each}
  </div>